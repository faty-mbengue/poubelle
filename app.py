...
# CSS personnalisé omitted for brevity, insérez votre CSS ici...
...
def predict_video_live(upload, frame_interval=1, stats_placeholder=None):
    ...
    # Contrôles Play/Pause et Vitesse
    ...
    with control_col3:
        ...
        selected_speed = st.select_slider("Vitesse", options=list(speed_options.keys()), value="1x")
    ...
    while True:
        ...
        if last_detection is not None:
            for box in last_detection.boxes:
                ...
                cv2.putText(display_frame, label_text, (x1+5, y1-8), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)

        timestamp_text = f"{minutes:02d}:{seconds:02d}"
        cv2.rectangle(display_frame, (5, 5), (150, 45), (0, 0, 0), -1)
        cv2.putText(display_frame, timestamp_text, (10, 35), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
